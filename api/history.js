...
const fetchHistory = async (address, chain, from_date, to_date) => {
    ...
    let page = 0;
    let all_txs = [];
    let url = `${baseURL}/wallets/${address}/history?chain=${chain}&nft_metadata=true&from_date=${from_date}`;
    if(to_date) {
        url = `${baseURL}/wallets/${address}/history?chain=${chain}&nft_metadata=true&from_date=${from_date}&to_date=${to_date}`;
    }
    console.log(`${baseURL}/wallets/${address}/history?chain=${chain}&nft_metadata=true&from_date=${from_date}&to_date=${to_date}`)
    do {
...
// FOr uniswap grouping
function addDateCategories(transactions) {
const now = new Date();
...
transactions.forEach(item => {
    const itemDate = new Date(item.block_timestamp);
    let dateCategory = "Older"; // Default category

    if (itemDate >= today) {
    dateCategory = "Today";
    } else if (itemDate >= yesterday && itemDate < today) {
    dateCategory = "Yesterday";
    } else if (itemDate >= thisMonthStart && itemDate < today) {
    dateCategory = "This Month";
    } else if (itemDate >= thisYearStart && itemDate < thisMonthStart) {
    dateCategory = "This Year";
    }

    // Directly add or update the date_category property of each item
    item.date_category = dateCategory;
});
}
