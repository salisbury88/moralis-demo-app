...
const Tokens = () => {
  ...
  const fetchTokens = (chain) => {
    ...
    fetch(`/api/wallet/tokens?chain=${chain}&wallet=${globalDataCache.walletAddress}`)
      .then(response => {
  ...
  const handleTokenClick = (token) => {
    ...
    fetch(`/api/wallet/tokens/${token.token_address}?chain=${globalDataCache.selectedChain}`)
      .then(response => {
        if (!response.ok) throw new Error('Failed to fetch data');
        return response.json();
      })
      ...
      <ul className="token-list">
        ...
        {globalDataCache.tokens && globalDataCache.tokens.filter(token => !token.possible_spam).map(token => (
            <li key={token.token_address} onClick={() => handleTokenClick(token)}>
              ...
              {token.price && (
                    <>
                        <div className="price">${Number(token.value).toFixed(2)}</div>
                    </>
              )}
              </div>
              <div className="token-value">
                ...
                <div className="container">                   
                      ...
                      <div className="token-modal-content">
                        ...
                        <ul className="table-list">
                          ...
                          <li>
                            ...
                            <div className="right">{utilities.shortAddress(activeToken.exchange?.address)}</div>
                          </li>                        
                        </ul>
                      </div>
                      ...
                      <div className="token-modal-content">
                          ...
                          <li>
                            ...
                            <div className="right">{activeToken.percentageChange}%</div>
                          </li>
                          <li>
                            <div className="left">7 Day Change (USD)</div>
                            <div className="right">${activeToken.usdChange}</div>
                          </li>                          
                        </ul>
                      </div>
                      ...
                      <div className="token-modal-content">
                          ...
                          <li>
                            ...
                            <div className="right">{utilities.shortAddress(activeToken.token.address)}</div>
                          </li>
                          <li>
                      ...
                      <div className="token-modal-content">
                          ...
                          <p>Minted at block {activeToken.token.block_number}.</p>
                      </div>
