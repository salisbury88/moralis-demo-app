                        ...
                        <div>
                            {item.erc20_transfers && item.erc20_transfers.filter(token => token.direction === "send").map(token => (
                                <div className="token-details send">
                                    ...
                                    <div>-{Number(token.value_decimal).toFixed(2)} {token.token_symbol}</div>
                                </div>
                            ))}
                            ...
                            <>
                               ...
                               <div>
                                    {item.erc20_transfers && item.erc20_transfers.filter(token => token.direction === "receive").map(token => (
                                        <div className="token-details receive">
                                            ...
                                            <div className="received">
                                                {token.value_decimal 
                                                ?
                                                    <>+{Number(token.value_decimal).toFixed(2)} {token.token_symbol}</>
                                                :
                                    ...
                                    {item.native_transfers && item.native_transfers.filter(token => token.direction === "receive").map(token => (
                                        <div className="token-details receive">
                                            <div><img src={`${token.token_logo}`}/></div>
                                            <div className="received">+{Number(token.value_decimal).toFixed(2)} {token.token_symbol}</div>
                                        </div>
                                    ))}
                    ...
                    
                        {
                            ...
                            <>
                                ...
                                <img 
                                    ...
                                    alt="profile"
                                    />
                                    {item.to_address_label ? item.to_address_label : item.to_address ? utilities.shortAddress(item.to_address) :utilities.shortAddress(item.from_address)}
                                </div>
                            </>
                        }
